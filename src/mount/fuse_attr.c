...
// --[ Description ] ----------------------------------------------------------
//
//     FUSE attribute and lifecycle operations for OBMAFS3.
//
// --[ License ] --------------------------------------------------------------
//
...
static int obmafs3_fuse_flush_impl(const char *path, struct fuse_file_info *fi)
{
    ...
    {
        int drc = obmafs3_flush_dedup_block_cache(g_ctx, ffctx->sector_size, &ffctx->db_cache);
        if(rc == OBMAFS3_OK) rc = drc;
    }

    /* Flush cached sector_map_entries for media image files */
    if(ffctx->sector_size && ffctx->sme_cache.count > 0)
    {
        int src = obmafs3_flush_sector_map_cache(g_ctx, &ffctx->inode, &ffctx->sme_cache);
        if(rc == OBMAFS3_OK) rc = src;
        ffctx->inode_dirty = 1;
    }
    ...
    {
        int crc = obmafs3_flush_cd_sector_map_cache(g_ctx, &ffctx->inode, &ffctx->cd_sme_cache);
        if(rc == OBMAFS3_OK) rc = crc;
        ffctx->inode_dirty = 1;
    }
...
static int obmafs3_fuse_statfs_impl(const char *path, struct statvfs *stbuf)
{
    ...
    uint64_t used = 0;
    for(uint64_t i = 0; i < g_ctx->bitmap_size; i++) used += (uint64_t)__builtin_popcount(g_ctx->bitmap[i]);
