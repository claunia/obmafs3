 ...
 *
 * Returns NULL for media types that don't have a CDRWin mapping.
 */
 ...
 */
static int write_cue_file(void *aaruf_ctx, const ImageInfo *info,
...
int main(int argc, char *argv[])
{
    ...
    /* ---- Export dump hardware as JSON if available ---- */
    {
        ...
        if(aaruf_get_dumphw(aaruf_ctx, NULL, &dumphw_size) == AARUF_ERROR_BUFFER_TOO_SMALL && dumphw_size > 0)
        {
            ...
            if(dumphw_buf)
            {
                if(aaruf_get_dumphw(aaruf_ctx, dumphw_buf, &dumphw_size) == AARUF_STATUS_OK &&
                   dumphw_size >= 18)
                {
                    /*
                     * Parse the raw binary block returned by aaruf_get_dumphw.
                    ...
                    if(jbuf && entries > 0)
                    {
                        ...
                        if(jbuf)
                        {
                            ...
                            if(hw_path)
                            {
                                ...
                                int hw_fd = open(hw_path, O_CREAT | O_WRONLY | O_EXCL, 0644);
                                if(hw_fd >= 0)
                                {
                                    ssize_t written = write(hw_fd, jbuf, pos);
                                    if(written < 0 || (size_t)written != pos)
                                        fprintf(stderr, "Warning: incomplete write of dump hardware JSON\n");
                                    else
                                        fprintf(stderr, "Dump hardware saved to %s\n", hw_path);
                                    close(hw_fd);
                                }
                                else
                                {
                                    fprintf(stderr, "Warning: failed to create '%s' (errno=%d: %s)\n",
                                            hw_path, errno, strerror(errno));
                                }
                                free(hw_path);
                            }
                            free(jbuf);
