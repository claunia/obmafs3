...
/**
 * Converts Aaru media type to human readable string.
 *
 * @param type  Aaru Media type.
 */
const char *aaru_media_type_to_string(MediaType type)
{
    switch(type)
    {
...
void import_metadata(void *aaruf_ctx, const ImageInfo *info, int fd)
{
    ...
    /* Store media type as a human readable string */
    {
        memset(&meta, 0, sizeof(meta));
        strncpy(meta.key, "MediaType", METADATA_KEY_MAX - 1);
