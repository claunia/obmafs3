    ...
    *out_count = 0;
    ...
    while (stk_size > 0) {
        ...
        for (uint16_t i = 0; i < hdr.node_keys; i++) {
            ...
            for (int e = 0; e < 8; e++) {
                if (rec.extents[e].block_count == 0)
                    continue;
                for (uint64_t b = 0; b < rec.extents[e].block_count;
                     b++) {
                    if (count >= cap) {
                        cap = (cap == 0) ? 128 : cap * 2;
        ...
        while (stk_size > 0) {
            ...
            for (uint16_t i = 0; i < nhdr.node_keys; i++) {
                ...
                if (!found) {
                    ...
                    int hrc = obmafs3_block_read(ctx, de.block_lba,
                                                 hdr_tmp,
                                                 sizeof(hdr_tmp));
                    ...
                    if (hrc == OBMAFS3_OK) {
                        ...
                        if (bh.magic == OBMAFS3_BLOCK_MAGIC) {
                            ...
                            if (used_std > std_per_dedup)
                                used_std = std_per_dedup;
                        }
                    }

                    /* Mark only the actually-used standard blocks */
                    for (uint64_t s = 0; s < used_std; s++)
                                      ...
                                      int *out_error)
{
    ...
    if (ctx->sb.refcount_lba != 0) {
        ...
        if (ctx->refcount_hdr.root_node_lba != 0) {
            ...
            uint64_t count = 0;
            int rc = walk_inode_btree_nodes(ctx,
                                            ctx->refcount_hdr.root_node_lba,
                                            &nodes, &count);
            if (rc == OBMAFS3_OK) {
                for (uint64_t i = 0; i < count; i++)
                    MARK(nodes[i]);
...
static uint64_t scrub_dedup_data_blocks(struct obmafs3_ctx *ctx)
{
    ...
    for (uint64_t i = 0; i < base_count; i++) {
        ...
        rc = obmafs3_block_read(ctx, bases[i], buf,
                                (size_t)ctx->sb.block_size);
        if (rc != OBMAFS3_OK) {
            read_errors++;
            bad++;
        ...
        if (needed_std > 1) {
            rc = obmafs3_block_read(ctx, bases[i] + 1,
                                    buf + bs,
...
static int compute_dedup_stats(struct obmafs3_ctx *ctx)
{
    ...
    struct tree_list_entry *tl_entries =
        malloc((size_t)(tree_count * sizeof(struct tree_list_entry)));
    if (!tl_entries) { free(list_buf); return OBMAFS3_ERR_NOMEM; }
    memcpy(tl_entries, list_buf + sizeof(struct tree_list_header),
           (size_t)(tree_count * sizeof(struct tree_list_entry)));
    free(list_buf);

    struct dedup_tree_stats *stats =
        calloc((size_t)tree_count, sizeof(*stats));
    if (!stats) { free(tl_entries); return OBMAFS3_ERR_NOMEM; }
        ...
        if (buf) {
            ...
            if (stack) {
                ...
                while (stk_sz > 0) {
                    ...
                    rc = obmafs3_block_read(ctx, lba, buf,
                                            (size_t)ctx->sb.block_size);
        ...
        if (base_count > 0) {
            ...
            if (hdr_buf) {
                for (uint64_t b = 0; b < base_count; b++) {
                    rc = obmafs3_block_read(ctx, bases[b], hdr_buf,
                                            (size_t)ctx->sb.block_size);
    ...
    for (uint64_t t = 0; t < tree_count; t++) {
        ...
        uint64_t dedup_sector_bytes =
            stats[t].dedup_entries * (uint64_t)stats[t].sector_size;
        ...
        if (stats[t].original_bytes > 0 &&
            stats[t].compressed_bytes < stats[t].original_bytes) {
            double comp_ratio = (double)stats[t].original_bytes /
                                (double)stats[t].compressed_bytes;
            double comp_saved = (1.0 - (double)stats[t].compressed_bytes /
                                       (double)stats[t].original_bytes)
                                * 100.0;
    ...
    if (tree_count > 1) {
        ...
        printf("    Unique sectors:         %" PRIu64 "\n",
               grand_dedup_entries);
        printf("    Dedup data blocks:      %" PRIu64
               " (%" PRIu64 " compressed, %" PRIu64 " uncompressed)\n",
               grand_unique_blocks, grand_compressed_blks,
               grand_uncompressed_blks);
        printf("    Uncompressed data:      ");
    ...
    if (total_sector_map_entries > 0 && grand_dedup_entries > 0) {
        double dedup_ratio = (double)total_sector_map_entries /
                             (double)grand_dedup_entries;
        uint64_t dup_sectors = total_sector_map_entries -
                               grand_dedup_entries;
        printf("    Dedup ratio:            %.2f:1"
               " (%" PRIu64 " refs -> %" PRIu64 " unique)\n",
               dedup_ratio, total_sector_map_entries,
               grand_dedup_entries);
        ...
        if (tree_count == 1) {
            uint64_t bytes_saved_dedup =
                dup_sectors * (uint64_t)stats[0].sector_size;
    ...
    if (grand_original > 0 && grand_compressed > 0) {
        double comp_ratio = (double)grand_original /
                            (double)grand_compressed;
    ...
    if (total_media_file_size > 0 && grand_physical > 0) {
        double overall = (double)total_media_file_size /
                         (double)grand_physical;
        ...
        if (total_media_file_size > grand_physical) {
            ...
            printf(" (%.1f%%)\n",
                   (double)total_saved /
                   (double)total_media_file_size * 100.0);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    if (ctx->inode_hdr.root_node_lba != 0) {
        ...
        int wrc = walk_inode_btree_nodes(ctx,
                                         ctx->inode_hdr.root_node_lba,
                                         &ino_nodes, &ino_node_count);
        if (wrc == OBMAFS3_OK) {
            uint64_t ino_bad = 0;
            verify_btree_node_checksums(ctx, ino_nodes, ino_node_count,
                                        "Inode", &ino_bad);
            free(ino_nodes);
            if (ino_bad > 0) {
                printf("  Node checksums:   %" PRIu64 " BAD\n",
    ...
    if (ctx->sb.refcount_lba != 0) {
        ...
        printf("  Magic:            0x%016" PRIx64 " (%s)\n",
               ...
               ctx->refcount_hdr.magic == OBMAFS3_BTREE_HDR_MAGIC
                   ? "OK" : "BAD");
        {
            int cs_ok = 0;
            obmafs3_btree_header_read_lenient(ctx, ctx->sb.refcount_lba,
                                              &ctx->refcount_hdr,
                                              &cs_ok);
            printf("  Header checksum:  %s\n", cs_ok ? "OK" : "BAD");
            if (!cs_ok) errors++;
    ...
    if (do_dedup_stats) {
        ...
        if (rc != OBMAFS3_OK)
            fprintf(stderr,
                    "Warning: could not compute dedup stats: %d\n", rc);
    }
