 ...
 */
static int refcount_leaf_find(const uint8_t *buf, uint16_t node_keys,
                              uint64_t lba)
{
    ...
    while (lo <= hi) {
        int mid = lo + (hi - lo) / 2;
        uint64_t mid_lba;
        memcpy(&mid_lba, data + (size_t)mid * sizeof(struct refcount_record),
               sizeof(mid_lba));
        if (mid_lba == lba)
            return mid;
        if (mid_lba < lba)
 ...
 */
static uint16_t refcount_index_find(const uint8_t *buf, uint16_t node_keys,
                                    uint64_t lba)
{
    const uint8_t *data = buf + sizeof(struct btree_node_header);
    int lo = 0, hi = (int)node_keys - 1;
                         ...
                         uint32_t *ref_count)
{
    ...
    while (1) {
        int rc = obmafs3_block_read(ctx, cur, buf,
                         ...
                         uint32_t ref_count)
{
            ...
            if (hdr.level > 0) {
                ...
                if (idx < hdr.node_keys - 1)
                    memmove(data + (size_t)idx * rec_sz,
                            ...
                            ((size_t)hdr.node_keys - (size_t)idx - 1)
                            * rec_sz);
                hdr.node_keys--;
                hdr.keys_length =
                    (uint16_t)(hdr.node_keys * rec_sz);
                /* Zero the tail */
    ...
    if (root_lba == 0) {
        ...
        return obmafs3_btree_header_write(ctx, ctx->sb.refcount_lba,
                                          &ctx->refcount_hdr);
    }
    ...
    while (1) {
        ...
        if (hdr.level == 0)
            break; /* reached leaf; buf holds it at cur_lba */
    ...
    if (idx >= 0) {
        memcpy(buf + sizeof(struct btree_node_header)
                   + (size_t)idx * sizeof(struct refcount_record),
               &new_rec, sizeof(new_rec));
        compute_node_checksum(buf);
    ...
    while (depth > 0) {
        ...
        if (phdr.node_keys < max_idx) {
            ...
            return obmafs3_btree_header_write(
                ctx, ctx->sb.refcount_lba, &ctx->refcount_hdr);
        }
