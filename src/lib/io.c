...
int obmafs3_open_flags(const char *path, int flags, struct obmafs3_ctx **ctx)
{
    ...
    if (flags & OBMAFS3_OPEN_LENIENT) {
        ...
        if (c->sb.cd_prefix_lba != 0) {
            rc = obmafs3_btree_header_read_lenient(c, c->sb.cd_prefix_lba,
                                                   &c->cd_prefix_hdr,
                                                   &cs_ok);
            if (rc != OBMAFS3_OK && rc != OBMAFS3_ERR_CHECKSUM) {
                close(fd); free(c); return rc;
        ...
        if (c->sb.cd_suffix_lba != 0) {
            rc = obmafs3_btree_header_read_lenient(c, c->sb.cd_suffix_lba,
                                                   &c->cd_suffix_hdr,
                                                   &cs_ok);
            if (rc != OBMAFS3_OK && rc != OBMAFS3_ERR_CHECKSUM) {
                close(fd); free(c); return rc;
        ...
        if (c->sb.cd_subchannel_lba != 0) {
            rc = obmafs3_btree_header_read_lenient(c, c->sb.cd_subchannel_lba,
                                                   &c->cd_subchannel_hdr,
                                                   &cs_ok);
            if (rc != OBMAFS3_OK && rc != OBMAFS3_ERR_CHECKSUM) {
                close(fd); free(c); return rc;
        ...
        if (c->sb.refcount_lba != 0) {
            rc = obmafs3_btree_header_read_lenient(c, c->sb.refcount_lba,
                                                   &c->refcount_hdr,
                                                   &cs_ok);
            if (rc != OBMAFS3_OK && rc != OBMAFS3_ERR_CHECKSUM) {
                close(fd); free(c); return rc;
        ...
        if (c->sb.cd_prefix_lba != 0) {
            rc = obmafs3_btree_header_read(c, c->sb.cd_prefix_lba,
                                           &c->cd_prefix_hdr);
            if (rc != OBMAFS3_OK) { close(fd); free(c); return rc; }
        }

        if (c->sb.cd_suffix_lba != 0) {
            rc = obmafs3_btree_header_read(c, c->sb.cd_suffix_lba,
                                           &c->cd_suffix_hdr);
            if (rc != OBMAFS3_OK) { close(fd); free(c); return rc; }
        }
        ...
        if (c->sb.refcount_lba != 0) {
            rc = obmafs3_btree_header_read(c, c->sb.refcount_lba,
                                           &c->refcount_hdr);
            if (rc != OBMAFS3_OK) { close(fd); free(c); return rc; }
        }
    }
                   ...
                   const char *label, const uint8_t *guid)
{
    ...
    /* --- Blocks 14..14+N-1: Allocation bitmap --- */
    {
        ...
        for (uint64_t i = 0; i < bitmap_blks; i++) {
            ...
            rc = write_block(fd, block_size, 14 + i, blk,
                             (size_t)block_size);
            if (rc != OBMAFS3_OK) {
                free(blk);
                free(bitmap);
