...
## Nintendo GameCube / Wii / Wii U Disc Image Support

OBMAFS3 supports importing Nintendo GameCube, Wii, and Wii U disc images with deduplication and — where applicable — junk detection and encryption handling, producing byte-identical output on read.

- **GameCube:** LFG junk detection and removal. No encryption.
- **Wii:** LFG junk detection and removal. AES-128-CBC decryption on import, re-encryption on read.
- **Wii U:** AES-128-CBC decryption on import, re-encryption on read. Per-partition title key derivation from on-disc tickets. No LFG junk (WUX-style sector dedup works because there's no deterministic padding).
...
Wii U discs use a different encryption scheme: the entire disc from offset 0x18000 onward is encrypted in independent 32 KiB sectors using AES-128-CBC with IV=zeros. Different partitions may use different keys (disc key for SI/UP/GI partitions, per-title key for GM game partitions). There is no LFG junk — unused space is typically all-zero, which deduplicates naturally.
...
kBtreeDataTypeJunkMapEntry = 13
kFileTypeNintendo       = 6    /* Nintendo GC/Wii/Wii U disc image */
...
```c
OBMAFS3_ROCOMPAT_NINTENDO  (1 << 1)   /* Nintendo GC/Wii/Wii U disc image support */
```
...
GameCube images are processed in 32 KB blocks. First, the entire block is tested with `ngc_lfg_get_seed(data, 0x8000, 0, seed)`. If it matches, each non-data sector (classified by FST) is zeroed and a junk entry recorded. For mixed blocks (junk/file/junk), the code finds contiguous runs of non-data sectors and attempts per-run seed extraction with the appropriate `data_offset`. Each sector in a matched run is then individually verified against the regenerated LFG output before being zeroed.
...
**Wii U:** `read_wiiu()`:
1. Reads the stored (decrypted) 0x8000-byte sector from dedup.
2. Determines if re-encryption is needed:
   - Sectors before 0x18000 (disc header area): returned as-is (plaintext).
   - Partition header sectors (identified by matching `data_offset` from per-partition metadata): returned as-is (plaintext).
   - All other sectors: re-encrypted with the correct partition key (looked up by sector offset within partition ranges) using AES-128-CBC with IV=zeros.
3. Returns the requested byte range from the (re-)encrypted sector.

Since AES-CBC decrypt followed by encrypt with the same key and IV is the identity transform, this produces byte-identical output to the original WUD/WUX.
...
| Key | Value |
|-----|-------|
| `__ngc_disc_type__` | `0` = GameCube, `1` = Wii, `2` = Wii U |
| `__ngc_disc_size__` | Total disc size in bytes |
| `__ngc_part_count__` | Number of partitions (0 for GC) |
| `__ngc_part_N_data_offset__` | Partition N data area disc offset |
| `__ngc_part_N_data_size__` | Partition N data area size |
| `__ngc_part_N_title_key__` | Partition N AES key (32 hex chars): disc key for SI/UP/GI, per-title key for GM |
...
### `import-wiiu` — Nintendo Wii U disc importer

Imports a Wii U WUD (raw) or WUX (sector-deduplicated) disc image into a mounted OBMAFS3 filesystem. Communicates via `open()`, `write()`, and `ioctl()` — does not link `libobmafs` beyond the shared library.

Usage: `import-wiiu <input.wud|wux> <output-path-on-obmafs>`
       `import-wiiu <input.wud|wux> <disc-key-file> <output-path-on-obmafs>`

If the disc key file argument is omitted, the tool automatically looks for a `.key` sidecar file next to the input image (e.g., `game.key` for `game.wud`).

**Import pipeline:**

1. **Phase 1:** Opens the image file (auto-detects WUX vs WUD), loads the 16-byte disc key from a sidecar file or explicit argument.
2. **Phase 2:** Decrypts the TOC sector (disc offset 0x18000) with the disc key (AES-128-CBC, IV=zeros). Verifies the TOC signature (`0xCCA6E67B`). Parses partition entries (identifier, name, start sector).
3. **Phase 3:** Scans SI and GI partitions for `TITLE.TIK` files by parsing their FSTs. Extracts encrypted title keys from tickets (`ticket+0x1BF`), title IDs (`ticket+0x1DC`), and decrypts them using the Wii U common key with IV = `title_id || 0x00*8`. Matches decrypted title keys to GM partitions by name (`"GM" + hex(title_id)`).
4. **Phase 4:** Creates the output file, sets it as `kFileTypeNintendo` with `disc_type=2` via `OBMAFS3_IOC_SET_NINTENDO_IMAGE` ioctl. Per-partition keys (disc key for SI/UP/GI, per-title key for GM) are stored in the partition descriptor.
5. **Phase 5:** Linear disc walk — for each 32 KiB sector:
   - Plaintext sectors (0-2 and partition headers): written verbatim.
   - Encrypted sectors: decrypted with the correct partition key (AES-128-CBC, IV=zeros), written as one 32 KiB `write()` call.
6. **Phase 6:** Stores the disc key as a media tag (`kNintendoWiiUDiscKey`, 16 bytes).
7. **Phase 7:** Stores metadata (ProductCode, GameID, Title, DiscNumber, DiscVersion, Region, MediaType, AaruMediaType, ImportApplication).

**WUX support:**

The WUX format is a simple sector-deduplication container. Header at offset 0:
- `uint32_t magic` — `"WUX0"` (0x30585557 LE)
- `uint32_t reserved`
- `uint32_t sector_size` — always 0x8000
- `uint32_t reserved2`
- `uint64_t uncompressed_size`

Followed by a `uint32_t[n_sectors]` index table mapping logical sectors to physical sectors. Data starts at the next sector-aligned offset after the index table. The `wiiu_reader` abstraction transparently handles WUX by translating logical offsets through the index table.

**Wii U disc encryption:**

All sectors from disc offset 0x18000 onward are AES-128-CBC encrypted with IV=zeros. Each 0x8000-byte sector is encrypted independently (no chaining between sectors). The encryption key differs by partition type:
- **SI (System Info), UP (Update), GI (Game Info):** Disc key (from the `.key` sidecar file).
- **GM (Game):** Per-title key derived from the partition's ticket using the Wii U common key.

Partition headers (first sector of each partition, identified by magic `0xCC93A4F5`) are always plaintext.

**Source files:**

| File | Contents |
|------|----------|
| `import_wiiu.h` | Shared header with constants, structures, and declarations |
| `main.c` | Entry point, argument parsing, key loading, phase orchestration |
| `disc.c` | WUD/WUX reader abstraction, disc info printing |
| `import.c` | TOC parsing, title key extraction from SI/GI FST tickets, sector import with per-partition decryption; Wii U common key |
| `metadata.c` | Disc metadata storage (product code, region, etc.) |

**New media tag:**

| Tag | Value | Description |
|-----|-------|-------------|
| `kNintendoWiiUDiscKey` | 76 | 16-byte Wii U disc key (from non-readable disc area) |
